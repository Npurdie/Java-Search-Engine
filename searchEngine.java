...
public class searchEngine {
    ...
    void traverseInternet(String url) throws Exception {
    	...
    	internet.setVisited(url,true);
    		//parse the current url and put it in the word index hashmap
    ...
    */
    void computePageRanks() {
    	...
    	for (int i=0; i<100; i++) {	//repeat 100 times for convergence
	    	...
	    	while (itr.hasNext()) {
				...
				while (itr2.hasNext()) {
					String s = itr2.next();
					temp = temp + internet.getPageRank(s)/internet.getOutDegree(s);
				}
				internet.setPageRank(vertex, 0.5+0.5*temp);
			}
		}
    ...
    */
    String getBestURL(String query) {
    	...
    	while (itr.hasNext()) {
			...
			if(internet.getPageRank(temp) > internet.getPageRank(highestRank))	{
				highestRank = temp;
			}
		}
	return highestRank;
